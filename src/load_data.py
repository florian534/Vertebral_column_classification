...
def load_raw_data(config_path):
    ...
    df = df.replace(b"Hernia", "Hernia")
    df = df.replace(b"Spondylolisthesis", "Spondylolisthesis")
    df = df.replace(b"Normal", "Normal")

    df = df.rename({"class": "target"}, axis=1)

    le = LabelEncoder()
    df["target"] = le.fit(df["target"]).transform(
        df["target"]
    )  # transformation label en 0,1 et 2
